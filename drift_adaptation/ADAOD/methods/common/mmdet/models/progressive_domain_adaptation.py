'''Progressive-DA discriminator, gradient reversal, and balanced DA loss.'''
...
def gradient_reverse(inputs: Tensor, scale: float = 1.0) -> Tensor:
    '''Return inputs while multiplying their backward gradient by -scale.'''
...
class ProgressiveDomainDiscriminator(nn.Module):
    '''Image-level discriminator used by the ADA-FNP comparison protocol.'''
...
def compute_multi_target_domain_loss(
    source_logits: Tensor,
    target_logits: Sequence[Tensor],
) -> Tensor:
    '''Average adversarial losses across available target branches.'''
