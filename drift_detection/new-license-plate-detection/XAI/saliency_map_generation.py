...
class ActivationsAndGradients:
    ...
    def form_hooks_yolo(self, target_layers):
        ...
        img_size = (640, 640)
        debug_shapes = os.getenv('YOLO_SHAPE_DEBUG', '0') == '1'

        def backward_hook(module, grad_input, grad_output):
            self.gradients['value'].append(grad_output[0])
            if debug_shapes:
                print(f"[SHAPE][GradCAM] target grad ({module.__class__.__name__}): {tuple(grad_output[0].shape)}")
            return None
        ...
        def forward_hook(module, input, output):
            ...
            if debug_shapes:
                if isinstance(output, torch.Tensor):
                    ...
                    out_shape = [tuple(x.shape) for x in output]
                else:
                    out_shape = str(type(output))
                print(f"[SHAPE][GradCAM] target activation ({module.__class__.__name__}): {out_shape}")
            return None
...
class BaseCAM:
    ...
    def forward(self,
        ...
        if self.model_algorithm== "YOLOv5":
            ...
            if self.debug_shapes:
                def _shape_info(v):
                    ...
                    return str(type(v))
                print(f"[SHAPE][XAI] predictions/logits/raw_feature_maps: {len(predictions)} / {len(logits)} / {len(x)}")
                if len(predictions) > 0:
                    print(f"[SHAPE][XAI] predictions summary: {_shape_info(predictions)}")
                if len(logits) > 0:
                    print(f"[SHAPE][XAI] logits summary: {_shape_info(logits)}")
                if len(objectivness) > 0:
                    print(f"[SHAPE][XAI] objectness summary: {_shape_info(objectivness)}")
        else:
        ...
        if self.uses_gradients:
            ...
            if self.model_algorithm == "YOLOv5":
                ...
                saliency_target_value = float(objectness_loss.detach().item())
        ...
        # or something else.
        cam_per_layer, cam_per_layer_pre_relu = self.compute_cam_per_layer(input_tensor,
                                                                            targets,
                                                                            eigen_smooth)
        self.last_cam_per_layer_pre_relu = cam_per_layer_pre_relu

        heat_map = self.aggregate_multi_layers(cam_per_layer)
        if len(cam_per_layer_pre_relu):
            # Keep an aggregated raw CAM before ReLU for optional analysis/saving.
            stacked_raw = np.concatenate(cam_per_layer_pre_relu, axis=1)
            self.last_cam_pre_relu = np.amax(stacked_raw, axis=1)
        else:
            self.last_cam_pre_relu = None
        self.init_gradient_and_activations()
        del cam_per_layer
        del cam_per_layer_pre_relu
        gc.collect()
        # torch.cuda.empty_cache()
        return heat_map,predictions,saliency_target_value
    ...
    def compute_cam_per_layer(
        ...
        for i in range(len(iterate_over)):
            ...
            cam = self.get_cam_image(input_tensor,
                                     ...
                                     layer_grads,
                                     eigen_smooth)
            # Preserve raw CAM before the ReLU clamp for later analysis.
            cam_raw = cam.copy()
            cam = np.maximum(cam, 0)
            scaled = scale_cam_image(cam, target_size)
            cam_per_target_layer.append(scaled[:, None, :])
            if target_size is not None:
                resized_raw = np.float32([cv2.resize(img, target_size) for img in cam_raw])
            else:
                resized_raw = np.float32(cam_raw)
            cam_per_target_layer_pre_relu.append(resized_raw[:, None, :])
        del activations_list
        if 'cam' in locals():
            del cam
        if 'cam_raw' in locals():
            del cam_raw
        if 'scaled' in locals():
            del scaled
        if 'resized_raw' in locals():
            del resized_raw
        return cam_per_target_layer, cam_per_target_layer_pre_relu
    ...
    def __call__(self,
        ...
        cam,prediction_dict,saliency_target_value = self.forward(input_tensor,targets, eigen_smooth)
        ...
        return cam,prediction_dict,saliency_target_value
