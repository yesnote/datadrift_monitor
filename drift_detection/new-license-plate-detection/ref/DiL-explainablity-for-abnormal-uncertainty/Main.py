...
class dil_interface():
    ...
    def XAI_plugin(self, input_images,file_names,output_path,base_model_preds,mode='clean'):
        ...
        complete_localization_score, complete_localization_list = saliency_sum(np.array(saliency_maps))
        saliency_maps_back = mask_objects_saliency_map(saliency_maps, base_model_preds)
        background_localization_score, background_localization_list = saliency_sum(np.array(saliency_maps_back))
...
def digital_evaluation_demo(evaluation_use_case,config):
    ...
    if evaluation_use_case!='adversarial_faster' and evaluation_use_case!='adversarial_yolo':
        images, file_names = load_dataset_attack_format(os.path.join(dataset_path, 'images'),
                                                        config.model_params['model_algorithm'])
...
if __name__ == "__main__":
    ...
    object_counter_path = os.path.join(dataset_path,'objects_count/objects_count.csv')
