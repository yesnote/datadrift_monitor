...
parser.add_argument('--model-path', type=str, default="yolov5x.pt", help='Path to the model')
...
def folder_main(folder_path):
    ...
    iou_confs = [0.45]
    for conf in confs:
        for iou_conf in iou_confs:
            print(f"Checking conf {conf} with iou {iou_conf}")
            output_dir = f'outputs/conf {conf} iou {iou_conf}'
            ...
            for idx,item in enumerate(os.listdir(folder_path)):
                ...
                torch_img = model.preprocessing(img[..., ::-1])
                tic = time.time()
                masks, logits, [boxes, class_idx, class_names, confidence] = saliency_method(torch_img)
                print("total time:", round(time.time() - tic, 4))
                result = torch_img.squeeze(0).mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).detach().cpu().numpy()
                result = result[..., ::-1]  # convert to bgr
                masks_exp = torch.cat(masks)
                heatmap_cam = torch.amax(masks_exp,0)
                heatmap_cam = heatmap_cam.detach().cpu().numpy()
                cam_image = show_cam_on_image(result/255, heatmap_cam.squeeze(0), use_rgb=True)
                image_with_bounding_boxes = cv2.cvtColor(cam_image, cv2.COLOR_BGR2RGB)
                img_name = split_extension(os.path.split(img_path)[-1], suffix='-res')
        ...
        #     res_img = put_text_box(bbox, cls_name, res_img)
        #     images.append(res_img)
        # output_path = f'{args.output_dir}/check_all.jpg'
