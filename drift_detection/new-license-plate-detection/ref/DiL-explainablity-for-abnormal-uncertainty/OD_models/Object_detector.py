...
class Object_detection_model():
    ...
    def load_COCO_model(self,decision_threshold,target_model_path):
        if self.algorithm == 'Faster_RCNN':
            ...
            model = YOLOV5TorchObjectDetector('yolov5x.pt', self.device, img_size=self.img_size, names=None,
    ...
    def load_SuperStore_model(self,model_params,decision_threshold,target_model_path):
        if self.algorithm == 'Faster_RCNN':
            ...
            model = YOLOV5TorchObjectDetector(target_model_path[0], self.device, img_size=self.img_size, names=SuperStore_classes_YOLO,
    ...
    def extract_predictions_YOLO(self,results,detection_threshold):
        boxes = results[0][0]
        labels = results[2][0]
        classes = self.transform_classes_from_yolo_to_faster(labels)
        scores = results[3][0]
