...
def saliency_sum(saliency):
    ...
    """
    num_of_saliency = saliency.shape[0] #The batch size / number of scaliency maps to calculate the score from
    final_score = torch.zeros(1)
    final_std_score = torch.zeros(1)
    margin = 0.0000001
    scores_list = []
    for idx, saliency_map in enumerate(saliency):
        nan_mask = np.isnan(saliency_map)  # Use the mask to set the nan values to 0
        saliency_map[nan_mask] = 0
        if not torch.is_tensor(saliency_map):  #TODO: This "if-else" part needs to be removed after all the saliency techniques will be changed to work only with torch.
...
def mask_objects_saliency_map(saliency_maps, prediction_dicts):
    ...
    for saliency_map,pred in zip(b_saliency_map, prediction_dicts):
        for bbox in pred['boxes']:
            saliency_map[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])] = torch.nan
