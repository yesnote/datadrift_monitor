...
class configs:
    ...
    def __init__(self, model_dataset="COCO"):
        ...
        self.COCO_dataset_params["images_dir"] = r"datasets/dataset_lp_small/images/val"
        self.COCO_dataset_params["annotations_path"] = (
            r"datasets/dataset_lp_small/annotations.json"
        )
        self.COCO_dataset_params["objects_count_path"] = (
            r"datasets/dataset_lp_small/objects_count.csv"
        )
        self.COCO_dataset_params["clean_validation_set_path"] = os.path.join(
            self.COCO_dataset_params["COCO_dataset_root_folder"], "Clean"
        )
        self.COCO_dataset_params["Unrealistic_PO"] = os.path.join(
            self.COCO_dataset_params["COCO_dataset_root_folder"], "Unrealistic PO"
        )
        self.COCO_dataset_params["Realistic_PO"] = os.path.join(
            self.COCO_dataset_params["COCO_dataset_root_folder"], "Realistic PO"
        )
        self.COCO_dataset_params["OOD"] = os.path.join(
            self.COCO_dataset_params["COCO_dataset_root_folder"], "OOD"
        )

        self.SuperStore_dataset_params = {
            "batch_size": 1,
            "img_size": 640,
            "super_store_dataset_root_folder": os.path.join(
                self.dataset_root_folder, "SuperStore"
            ),
        }
        self.SuperStore_dataset_params["clean_validation_set_path"] = os.path.join(
            self.SuperStore_dataset_params["super_store_dataset_root_folder"], "Clean"
        )
        self.SuperStore_dataset_params["partial_occlusion_validation_set_path"] = (
            os.path.join(
                self.SuperStore_dataset_params["super_store_dataset_root_folder"], "PO"
            )
        )
        self.SuperStore_dataset_params["OOD_validation_set_path"] = os.path.join(
            self.SuperStore_dataset_params["super_store_dataset_root_folder"], "OOD"
        )
        self.SuperStore_dataset_params["adversarial_faster_validation_set_path"] = (
            os.path.join(
                self.SuperStore_dataset_params["super_store_dataset_root_folder"],
                "Adv faster rcnn",
            )
        )
        self.SuperStore_dataset_params["adversarial_yolo_validation_set_path"] = (
            os.path.join(
                self.SuperStore_dataset_params["super_store_dataset_root_folder"],
                "Adv_yolo",
            )
        ...
        if self.model_params["model_algorithm"] == "Faster_RCNN" or "MMdetection":
            self.attack_params["patch_path"] = self.attack_params["patches"][
                "faster_patch_coco"
            ]
        else:
            self.attack_params["patch_path"] = self.attack_params["patches"][
    ...
    def init_model_zoo(self):
        ...
        super_store_model_zoo["faster_rcnn_seed_42"] = [
            os.path.join(
                super_store_models_root_path, "faster_rcnn/target_model_rcnn_seed_42.pt"
            )
        ]
        super_store_model_zoo["yolov5f"] = [
            os.path.join(super_store_models_root_path, "YOLOv5/finetune/best.pt")
        ]

        super_store_models_root_path = os.path.join(
            models_root_folder, "mmdetection/configs"
        )
        super_store_model_zoo["faster_rcnn_caffe_fpn_50"] = [
            os.path.join(
                super_store_models_root_path,
                "faster_rcnn/faster_rcnn_r50_caffe_fpn_mstrain_3x_superstore.pth",
            ),
            os.path.join(
                super_store_models_root_path,
                "faster_rcnn/faster_rcnn_r50_caffe_fpn_mstrain_3x_superstore.py",
            ),
        ]
        super_store_model_zoo["grid_rcnn"] = [
            os.path.join(
                super_store_models_root_path,
                "grid_rcnn/grid-rcnn_r50_fpn_gn-head_2x_superstore.pth",
            ),
            os.path.join(
                super_store_models_root_path,
                "grid_rcnn/grid-rcnn_r50_fpn_gn-head_2x_superstore.py",
            ),
        ]
        ...
        super_store_model_zoo["double_heads_faster_rcnn"] = [
            os.path.join(
                super_store_models_root_path,
                "double_heads/dh-faster-rcnn_r50_fpn_1x_superstore.pth",
            ),
            os.path.join(
                super_store_models_root_path,
                "double_heads/dh-faster-rcnn_r50_fpn_1x_superstore.py",
            ),
        ]
        ...
        super_store_model_zoo["cascade_rpn"] = [
            os.path.join(
                super_store_models_root_path,
                "cascade_rpn/cascade-rpn_faster-rcnn_r50-caffe_fpn_1x_superstore.pth",
            ),
            os.path.join(
                super_store_models_root_path,
                "cascade_rpn/cascade-rpn_faster-rcnn_r50-caffe_fpn_1x_superstore.py",
            ),
        ]
        ...
        super_store_model_zoo["cascade_rcnn"] = [
            os.path.join(
                super_store_models_root_path,
                "cascade_rcnn/cascade-rcnn_r50_fpn_1x_superstore.pth",
            ),
            os.path.join(
                super_store_models_root_path,
                "cascade_rcnn/cascade-rcnn_r50_fpn_1x_superstore.py",
            ),
        ]

        super_store_model_zoo["yolox_x"] = [
            os.path.join(
                super_store_models_root_path, "yolox/yolox_x_8xb8-300e_superstore.pth"
            ),
            os.path.join(
                super_store_models_root_path, "yolox/yolox_x_8xb8-300e_superstore.py"
            ),
        ]
        ...
        super_store_model_zoo["yolo_f"] = [
            os.path.join(
                super_store_models_root_path,
                "yolof/yolof_r50-c5_8xb8-1x_superstore.pth",
            ),
            os.path.join(
                super_store_models_root_path, "yolof/yolof_r50-c5_8xb8-1x_superstore.py"
            ),
        ]
        ...
        super_store_model_zoo["yolo_v3"] = [
            os.path.join(
                super_store_models_root_path,
                "yolo/yolov3_d53_8xb8-ms-608_superstore.pth",
            ),
            os.path.join(
                super_store_models_root_path,
                "yolo/yolov3_d53_8xb8-ms-608_superstore.py",
            ),
        ]
        ...
        COCO_model_zoo["faster_rcnn_r50_fpn"] = [
            os.path.join(
                COCO_models_root_path, "faster_rcnn/faster-rcnn_r50_fpn_2x_coco.pth"
            ),
            os.path.join(
                COCO_models_root_path, "faster_rcnn/faster-rcnn_r50_fpn_2x_coco.py"
            ),
        ]
        COCO_model_zoo["cascade_rpn"] = [
            os.path.join(
                COCO_models_root_path,
                "cascade_rpn/cascade-rpn_faster-rcnn_r50-caffe_fpn_1x_coco_test.pth",
            ),
            os.path.join(
                COCO_models_root_path,
                "cascade_rpn/cascade-rpn_faster-rcnn_r50-caffe_fpn_1x_coco_test.py",
            ),
        ]
        COCO_model_zoo["cascade_rcnn"] = [
            os.path.join(
                COCO_models_root_path, "cascade_rcnn/cascade_rcnn_r101_fpn_1x_coco.pth"
            ),
            os.path.join(
                COCO_models_root_path, "cascade_rcnn/cascade_rcnn_r101_fpn_1x_coco.py"
            ),
        ]
        COCO_model_zoo["double_heads_faster_rcnn"] = [
            os.path.join(
                COCO_models_root_path, "double_heads/dh-faster-rcnn_r50_fpn_1x_coco.pth"
            ),
            os.path.join(
                COCO_models_root_path, "double_heads/dh-faster-rcnn_r50_fpn_1x_coco.py"
            ),
        ]
        COCO_model_zoo["grid_rcnn"] = [
            os.path.join(
                COCO_models_root_path, "grid_rcnn/grid-rcnn_r50_fpn_gn-head_2x_coco.pth"
            ),
            os.path.join(
                COCO_models_root_path, "grid_rcnn/grid-rcnn_r50_fpn_gn-head_2x_coco.py"
            ),
        ]
        ...
        COCO_model_zoo["yolo_v3"] = [
            os.path.join(
                COCO_models_root_path, "yolo/yolov3_d53_8xb8-320-273e_coco.pth"
            ),
            os.path.join(
                COCO_models_root_path, "yolo/yolov3_d53_8xb8-320-273e_coco.py"
            ),
        ]
    ...
    def get_patches(self):
        ...
        patches = {}
        patches["faster_patch_coco"] = os.path.join(
            patches_root_path, "faster_patch_coco/best_patch.npy"
        )
        patches["faster_patch_super_store"] = os.path.join(
            patches_root_path, "faster_patch_super_store/best_patch.npy"
        )
        patches["yolo_patch_coco"] = os.path.join(
            patches_root_path, "yolo_patch_coco/best_patch.npy"
        )
        patches["yolo_patch_super_store"] = os.path.join(
            patches_root_path, "yolo_patch_super_store/best_patch.npy"
        )
        patches["RandomNoisePatch"] = os.path.join(
            patches_root_path, "random_noise_patch/best_patch.npy"
        )
        return patches
