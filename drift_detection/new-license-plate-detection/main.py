...
class dil_interface:
    ...
    def dil_experiment(self, images, file_names, object_counter_path, annotations):
        ...
        """
        self.eval_module = evaluation_module(
            self.od_model,
            self.model_params,
            self.dataset_params,
        ...
        eval_dict = self.eval_module.create_evaluation_dict(
            images, file_names, gt_objects_counter
        )
        predictions = self.predict_and_evaluate_model(
            images,
            file_names,
            gt_objects_counter,
        ...
        images_fool, self.file_names_fool, predictions_fool, images_dropped = (
            self.drop_good_predictions(
                images, file_names, predictions, gt_objects_counter
            )
        )
        self.eval_module.mAP(
            np.array(predictions_fool),
            annotations,
            self.file_names_fool,
            images_fool,
            self.model_params["model_dataset"],
        )
        distinctive_localization_scores, saliency_maps = self.XAI_plugin(
            images,
            file_names,
        ...
        print(
            "Model predictions and their corresponding saliency maps are saved in the Experiment_output folder"
        )

    def dil_clean_experiment(
        self, images, file_names, object_counter_path, annotations
    ):
        """
        Main function that establish an experiment on DiL estimation.
        :param images: req. list of scenes in tensor format.
    ...
    def compute_ddt(self, images, file_names, object_counter_path, annotations):
        ...
        for image_id, dil_score in zip(
            eval_dict.keys(), distinctive_localization_scores
        ):
        ...
        self.predict_and_evaluate_model(
            images_fool,
            self.file_names_fool,
            gt_objects_counter,
    ...
    def XAI_plugin(
        self, input_images, file_names, output_path, base_model_preds, mode="clean"
    ):
        """
        ...
        """
        exp = Explainer(
            object_detection_model=self.xai_model,
        ...
        distinctive_localization_score = [
            background_score / objectness_score
            for objectness_score, background_score in zip(
        ...
        if mode == "clean":
            ...
            print(
                f"Mean background localization score: {background_localization_score}"
            )
            print(
                f"Mean DiL score :{sum(distinctive_localization_score) / len(distinctive_localization_score)}"
            )
        # Drop samples were the attack failed
        else:
            filtered_complete, filtered_cackground, filtered_dils = [], [], []
            dil_t = 0.3
            for complete_score, background_score, dil_score, file_name in zip(
            ...
            print(
                f"Mean complete localization score: {sum(filtered_complete) / len(filtered_complete)}"
            )
            print(
    ...
    def generate_dynamic_threshold(self, dil_scores):
        ...
        """
        dynamic_thresholds = [
            self.calculate_threshold(confidence_threshold)
            for confidence_threshold in dil_scores
        ]
    ...
    def calculate_threshold(self, dil_score):
        ...
        if math.isnan(dil_score):
            dil_score = 1.0
        return round(
            self.model_params["decision_threshold"]
            - (self.robustness_params["Maximum_degradation"] * dil_score),
            3,
        )

    def plot(self, images, prediction_dicts, file_names, output_path=None):
        images = [
            patched_image.detach().cpu().numpy() * 255 for patched_image in images
        ]
        for idx, (image, pred_dict, file_name) in enumerate(
            zip(images, prediction_dicts, file_names)
        ):
    ...
    def predict(
        self, images, file_names, output_path, detection_threshold=None, DiL_scores=None
    ):
        """
        ...
        preds = self.od_model.predict_wrapper(
            image_dataloader=images,
            saliency_maps=None,
            DiL_scores=DiL_scores,
            use_grad=False,
    ...
    def predict_and_evaluate_model(
        ...
        pred_objects_counter, predictions = dil.predict(
            images, file_names, output_plot_path, detection_threshold
        )
    ...
    def evaluate_recall_and_fp(self, gt_objects_counter, pred_objects_counter):
        gt_objects_counter, pred_objects_predicted = self.extract_object_counts(
            gt_objects_counter, pred_objects_counter
        )
        model_performance = evaluation_module.get_base_model_performance(
            gt_objects_counter, pred_objects_predicted
        )
        ...
        if self.pred_objects_predicted != None:
            dif = evaluation_module.get_base_robustness_models_performance_difference(
                gt_objects_counter, self.pred_objects_predicted, pred_objects_predicted
            )
            print(f"Base robustness models performance difference :{dif}")
        else:
            self.pred_objects_predicted = pred_objects_predicted

    def drop_good_predictions(
        self, images, file_names, predictions, gt_objects_counter
    ):
        """
        ...
        index_to_keep = []
        index_to_remove = []
        for idx, (prediction, gt_object_counter) in enumerate(
            zip(predictions, gt_objects_counter)
        ):
...
def generate_attack(config):
    ...
    (
        images,
        patched_images,
        benign_prediction_dicts,
        adversarial_prediction_dicts,
        file_names,
    ) = attack_module.apply_digital_attack()
    return (
        images,
        patched_images,
        benign_prediction_dicts,
...
def digital_evaluation_demo(evaluation_use_case, config):
    ...
    if evaluation_use_case == "clean":
        ...
        evaluation_use_case == "adversarial_faster"
        or evaluation_use_case == "adversarial_yolo"
    ):
        dataset_path = config.dataset_params["clean_validation_set_path"]
        (
            images,
            ...
            adversarial_prediction_dicts,
            file_names,
        ) = generate_attack(config)
        images = patched_images
    else:
        print(
            "You choose an invalid evaluation use case for the digital evaluation space."
        )
        return

    if (
        evaluation_use_case != "adversarial_faster"
        and evaluation_use_case != "adversarial_yolo"
    ):
        images_dir = config.dataset_params.get(
            "images_dir", os.path.join(dataset_path, "images")
        )
        images, file_names = load_dataset_attack_format(
            images_dir, config.model_params["model_algorithm"]
        )
...
def physical_evaluation_demo(evaluation_use_case, config):
    ...
    if evaluation_use_case == "clean":
        ...
        print(
            f"Physical evaluation space do not support {evaluation_use_case} use case, "
...
if __name__ == "__main__":
    ...
    evaluation_use_case_options = [
        "clean",
        "unrealistic partial occlusion",
        "realistic partial occlusion",
        "ood",
        "adversarial_faster",
        "adversarial_yolo",
    ]
    ...
    evaluation_use_case = evaluation_use_case_options[3]

    #  Upload the images and their corresponding file ids from the chosen dataset.
    if evaluation_space == "digital":
        config = configs("COCO")
        images, file_names, dataset_path = digital_evaluation_demo(
            evaluation_use_case, config
        )
    else:
        config = configs("SuperStore")
        images, file_names, dataset_path = physical_evaluation_demo(
            evaluation_use_case, config
        )
    ...
    object_counter_path = config.dataset_params.get(
        "objects_count_path",
        os.path.join(dataset_path, "objects_count/objects_count.csv"),
    )
    annotations_path = config.dataset_params.get(
        "annotations_path", os.path.join(dataset_path, "annotations/annotations.json")
    )

    with open(annotations_path) as json_file:
        annotations = json.load(json_file)
    if evaluation_use_case == "clean":
        dil.dil_clean_experiment(
            images,
            file_names,
            object_counter_path=object_counter_path,
            annotations=annotations,
        )
    else:
        dil.dil_experiment(
