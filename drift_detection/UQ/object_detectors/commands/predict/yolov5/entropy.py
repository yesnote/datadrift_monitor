    ...
    with open(output_csv, "w", newline="", encoding="utf-8") as output_file:
        ...
        for images, targets in tqdm(
            ...
            for sample_idx in range(len(image_list)):
                ...
                selected_probs = selected_softmax_class_probs(
            ...
            for item in iter_yolo_detection_rows(
                ...
                batch_items += 1
                pred_entropy = entropy_by_sample[item["sample_idx"]]
                pred_idx = item["pred_idx"]
                row = dict(item["base_row"])
                row["entropy"] = float(pred_entropy[pred_idx].detach().cpu().item()) if pred_idx < pred_entropy.shape[0] else 0.0
                writer.writerow(row)
