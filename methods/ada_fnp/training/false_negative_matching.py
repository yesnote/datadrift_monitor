...
def _compute_pairwise_iou(
    ...
    return intersection / union.clamp_min(
...
def count_false_negatives(
    ...
    if ground_truth_ignore is None:
        ground_truth_ignore = torch.zeros_like(
            ground_truth_labels, dtype=torch.bool
        )
    if len(ground_truth_ignore) != len(ground_truth_boxes):
        raise ValueError('ground_truth_ignore length mismatch')
    valid_ground_truth = ~ground_truth_ignore.bool()
    boxes = ground_truth_boxes[valid_ground_truth]
    labels = ground_truth_labels[valid_ground_truth]
    if len(boxes) == 0:
        return 0
    ...
    pairwise_ious = _compute_pairwise_iou(prediction_boxes[order], boxes)
    ...
    for prediction_index, original_index in enumerate(order):
        ...
        candidate_ious = pairwise_ious[prediction_index, candidate_indices]
