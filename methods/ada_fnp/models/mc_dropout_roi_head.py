...
from methods.ada_fnp.acquisition.scoring import normalize_xyxy_box_samples

P = TypeVar('P')
R = TypeVar('R')


def summarize_monte_carlo_predictions(
    probability_samples: Tensor, box_samples: Tensor
    ...
    if (
        box_samples.ndim == 4
        and box_samples.shape[2] != probability_samples.shape[2] - 1
    ):
        raise ValueError(
...
def split_multiclass_nms_indices(
    kept_flat_indices: Tensor, num_foreground_classes: int
) -> Tuple[Tensor, Tensor]:
...
class AdaFnpMonteCarloDropoutRoIHead(StandardRoIHead):
    ...
    def predict_fixed_proposals(
        ...
        for image_index, data_sample in enumerate(batch_data_samples):
            ...
            result.class_probabilities = mean_probabilities[proposal_indices]
