...
from models.meta_regressor import build_estimator
...
def _evaluate_feature_set(
    ...
    if process == "kfold":
        ...
        splitter = KFold(n_splits=num_fold, shuffle=True, random_state=random_seed)
    ...
    for train_idx, test_idx in iterator:
        ...
        y_pred = np.clip(estimator.predict(x[test_idx]), 0.0, 1.0)
        metrics = evaluate_regressor(y[test_idx], y_pred)
...
def _set_r2_ylim(ax, values: np.ndarray) -> None:
...
def _plot_single_feature(results_df: pd.DataFrame, out_path: Path) -> None:
    df = results_df.sort_values("r2_mean", ascending=False).reset_index(drop=True)
    x = np.arange(len(df))
    fig_width = max(12.0, 0.18 * len(df) + 4.0)
    fig, ax = plt.subplots(figsize=(fig_width, 6.0))
    ax.plot(x, df["r2_mean"], color="#4C78A8", linewidth=1.8, marker="o", markersize=2.5, label="R2")
    _set_r2_ylim(ax, df["r2_mean"].to_numpy(dtype=np.float64))
    ax.set_ylabel("Regression performance (R2)")
    ax.set_xlabel("Feature")
    ax.set_title("Single MetaDetect Feature R2")
    ax.set_xticks(x)
...
def _plot_cumulative(results_df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.plot(results_df["num_features"], results_df["r2_mean"], label="R2", linewidth=2.0)
    _set_r2_ylim(ax, results_df["r2_mean"].to_numpy(dtype=np.float64))
    ax.set_xlabel("Number of features")
    ax.set_ylabel("Regression performance (R2)")
    ax.set_title("Cumulative MetaDetect Feature R2")
...
def _plot_cumulative_delta(results_df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.plot(x, df["delta_r2"], label="Delta R2", color="#4C78A8", linewidth=1.8, marker="o", markersize=3)
    y_values = df["delta_r2"].to_numpy(dtype=np.float64)
...
def run_feature_ablation(config: dict[str, Any], run_dir: Path) -> Path:
    ...
    df, label_col, feature_columns, root_info = load_training_dataframe(dataset_cfg, task="regressor")
    ...
    single_df = pd.DataFrame(single_rows).sort_values("r2_mean", ascending=False).reset_index(drop=True)
    ...
    metadata = {
        ...
        "model": str(model_cfg.get("type", "gb_regressor")),
        "feature_dimension": len(feature_columns),
        "feature_group_count": len(feature_groups),
        "num_rows": int(len(df)),
        "target_mean": float(df[label_col].astype(float).mean()),
        "target_std": float(df[label_col].astype(float).std(ddof=0)),
