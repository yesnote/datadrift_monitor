...
UNTO_O_ORDER = ['Full UnTO-O', 'Final-detection only', 'Target-relative only', 'Final class+score', 'Final localization', 'Target class', 'Target score', 'Target localization']
GRAD_COMPONENTS = {'Localization gradient only': 't-null__term-bbox__b-box_l1-pred', 'Classification gradient only': 't-null__term-cls__c-kl-pred', 'Detection-score gradient only': 't-null__term-obj__o-bce-pred'}
...
def unto_o_feature_sets(cols: list[str]) -> dict[str, list[str]]:
    ...
    target_class = ['cls_loss'] if 'cls_loss' in cols else []
    target_score = ['obj_loss'] if 'obj_loss' in cols else []
    target_loc = [c for c in ['x_loss', 'y_loss', 'w_loss', 'h_loss', 'size_diff', 'circum_diff', 'size_circum_diff'] if c in cols]
    return {'Full UnTO-O': native_features(cols, 'null_detect'), 'Final-detection only': final_class + final_score + final_loc, 'Target-relative only': target_class + target_score + target_loc, 'Final class+score': final_class + final_score, 'Final localization': final_loc, 'Target class': target_class, 'Target score': target_score, 'Target localization': target_loc}

def slug(text: str) -> str:
    return re.sub(r'[^a-z0-9]+', '_', text.lower().replace('+', 'plus')).strip('_') or 'variant'

def feature_root_base(output_dir: Path, dataset_key: str) -> Path:
    return output_dir / 'feature_roots/object_detectors/runs/yolov5/predict' / dataset_key

def materialize_root(source_root: Path, csv_name: str, cue: str, dataset_key: str, tail: str, output_dir: Path, feature_cols: list[str] | None, limit_rows: int | None) -> Path:
    ...
    target = feature_root_base(output_dir, dataset_key) / f'00-00-0000_00;00_{cue}_{tail}'
...
def maybe_limit_root(source_root: Path, csv_name: str, cue: str, dataset_key: str, tail: str, output_dir: Path, limit_rows: int | None) -> Path:
    if limit_rows is None:
        return source_root
    return materialize_root(source_root, csv_name, cue, dataset_key, tail, output_dir, None, limit_rows)
...
def run_variant(dataset: DatasetConfig, variant: str, group: str, input_roots: Path | list[Path], classifier_base: dict[str, Any], regressor_base: dict[str, Any], output_dir: Path) -> EvalResult:
    run_root = output_dir / 'runs' / dataset.key / slug(group) / slug(variant)
    ...
    print(f'[{dataset.display_name}][{group}] {variant}: classifier')
    run_classifier_train(cls_cfg, cls_dir)
    print(f'[{dataset.display_name}][{group}] {variant}: regressor')
    ...
    return EvalResult(dataset.display_name, variant, rows, int(cls_meta.get('feature_dimension', 0)), float(tp / rows) if rows else 0.0, float(cls_mean['auroc']), float(cls_std['auroc']), float(cls_mean['ap']), float(cls_std['ap']), float(cls_mean['fpr95']), float(cls_std['fpr95']), float(reg_mean['r2']), float(reg_std['r2']), rel(cls_dir), rel(reg_dir))
...
def evaluate_unto_o(dataset: DatasetConfig, classifier_base: dict[str, Any], regressor_base: dict[str, Any], output_dir: Path, limit_rows: int | None) -> list[EvalResult]:
    ...
    results = []
    for variant in UNTO_O_ORDER:
        features = sets[variant]
        if not features:
            raise ValueError(f'Empty UnTO-O subset: {dataset.display_name} / {variant}')
        if variant == 'Full UnTO-O' and limit_rows is None:
            root = dataset.unto_o_root
        else:
            root = materialize_root(dataset.unto_o_root, 'null_detect.csv', 'null_detect', dataset.key, slug(variant), output_dir, None if variant == 'Full UnTO-O' else features, limit_rows)
        results.append(run_variant(dataset, variant, 'UnTO-O ablation', root, classifier_base, regressor_base, output_dir))
    return results
...
def evaluate_unto_g(dataset: DatasetConfig, classifier_base: dict[str, Any], regressor_base: dict[str, Any], output_dir: Path, limit_rows: int | None) -> list[EvalResult]:
    roots = {}
    for name, suffix in GRAD_COMPONENTS.items():
        root = find_component_root(dataset.grad_grid_root, suffix)
        roots[name] = maybe_limit_root(root, 'layer_grad.csv', 'layer_grad', dataset.key, suffix, output_dir, limit_rows)
    roots['All components'] = [roots[name] for name in GRAD_COMPONENTS]
    order = ['Localization gradient only', 'Classification gradient only', 'Detection-score gradient only', 'All components']
    return [run_variant(dataset, name, 'UnTO-G component ablation', roots[name], classifier_base, regressor_base, output_dir) for name in order]
...
def write_ablation_table(path: Path, label: str, caption: str, results: list[EvalResult]) -> None:
    lines = [r'\begin{table}[t]', r'\centering', rf'\caption{{{caption}}}', rf'\label{{{label}}}', r'\begin{tabular}{L{0.13\linewidth}L{0.24\linewidth}ccccc}', r'\toprule', r'Dataset & Variant & \# Feat. & AUROC & AP & FPR95 & $R^2$ \\', r'\midrule']
    prev = None
    for r in results:
        ds = esc(r.dataset) if r.dataset != prev else ''
        prev = r.dataset
        lines.append(f'{ds} & {esc(r.name)} & {r.num_features} & {fmt(r.auroc_mean, r.auroc_std)} & {fmt(r.ap_mean, r.ap_std)} & {fmt(r.fpr95_mean, r.fpr95_std)} & {fmt(r.r2_mean, r.r2_std, False)} \\\\')
...
def write_gradient_layers_table(path: Path) -> None:
    ...
    for detector, component, layers in GRAD_LAYER_ROWS:
        lines.append(f'{esc(detector)} & {esc(component)} & {esc(layers)} ' + r'\\')
    lines.extend([r'\bottomrule', r'\end{tabular}', r'\end{table}'])
    path.write_text('\n'.join(lines) + '\n', encoding='utf-8')
...
def write_dataset_summary(path: Path, rows: list[dict[str, Any]], limit_rows: int | None) -> None:
    suffix = ' on the loaded evaluation rows' + (f' (limited to {limit_rows:,} rows for a dry run)' if limit_rows else '')
    lines = [r'\begin{table}[t]', r'\centering', rf'\caption{{YOLOv5 appendix ablation data summary{suffix}.}}', r'\label{tab:app_dataset_summary}', r'\begin{tabular}{lccc}', r'\toprule', r'Dataset & Detections & TP Ratio & Regression Target \\', r'\midrule']
    for row in rows:
        lines.append(f'{esc(str(row["display_name"]))} & {int(row["detections"]):,} & {float(row["tp_ratio"]) * 100:.2f}\\% & best same-class IoU ' + r'\\')
    lines.extend([r'\bottomrule', r'\end{tabular}', r'\end{table}'])
    path.write_text('\n'.join(lines) + '\n', encoding='utf-8')
...
def selected_configs(args: argparse.Namespace) -> list[DatasetConfig]:
    ...
    if any(roots):
        ...
        return [DatasetConfig('custom', args.custom_dataset_name, args.custom_dataset_name, resolve_path(args.gt_root), resolve_path(args.unto_o_root), resolve_path(args.grad_grid_root))]
...
def main() -> None:
    ...
    metadata: dict[str, Any] = {'selected_datasets': [d.key for d in datasets], 'limit_rows': args.limit_rows, 'classifier_config_path': rel(classifier_path), 'regressor_config_path': rel(regressor_path), 'regressor_model_override': 'xgb_regressor', 'datasets': {}}
    ...
    for dataset in datasets:
        ...
        first = next((r for r in unto_o_results + unto_g_results if r.dataset == dataset.display_name), None)
        if first:
            dataset_rows.append({'detector': 'YOLOv5', 'dataset': dataset.summary_name, 'display_name': dataset.display_name, 'detections': first.num_rows, 'tp_ratio': first.tp_ratio, 'regression_target': 'best same-class IoU', 'limit_rows': args.limit_rows or ''})
        metadata['datasets'][dataset.key] = {'display_name': dataset.display_name, 'gt_root': str(dataset.gt_root), 'unto_o_root': str(dataset.unto_o_root), 'grad_grid_root': str(dataset.grad_grid_root)}
    ...
    if unto_o_results:
        ...
        write_ablation_table(output_dir / 'unto_o_ablation_table.tex', 'tab:app_unto_o_ablation', 'UnTO-O indicator ablation on YOLOv5 using MS COCO and Pascal VOC.', unto_o_results)
    if unto_g_results:
        pd.DataFrame([result_dict(r) for r in unto_g_results]).to_csv(output_dir / 'unto_g_component_ablation_results.csv', index=False)
        write_ablation_table(output_dir / 'unto_g_component_table.tex', 'tab:app_unto_g_component_ablation', 'UnTO-G component-gradient ablation on YOLOv5 using MS COCO and Pascal VOC.', unto_g_results)
